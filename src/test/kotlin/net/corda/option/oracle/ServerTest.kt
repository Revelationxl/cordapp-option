...
class OracleServiceTests {
    ...
    fun setUp() {
        ...
        spotOf = SpotOf("IBM", Instant.parse("2017-07-03T10:15:30.00Z"))
    }
    ...
    fun `successful query`() {
        database.transaction {
            val result = oracle.query(spotOf)
    ...
    fun `successful sign`() {
        database.transaction {
            val spot = Spot(spotOf, 3.DOLLARS)
    ...
    fun `incorrect spot specified`() {
        database.transaction {
            val spot = Spot(spotOf, 20.DOLLARS)
            val command = Command(OptionContract.Commands.Exercise(spot), listOf(CHARLIE.owningKey))
            val state = getOption()
