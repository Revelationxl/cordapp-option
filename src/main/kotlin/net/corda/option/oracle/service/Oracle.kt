...
// to an instance which should already exist on the stack.
    ...
    class Oracle(val identity: Party, val services: ServiceHub) : SingletonSerializeAsToken() {
        ...
        private val spots: List<Spot> = loadSpotPrices()

        // For now, load spots from file and parse into list
        fun loadSpotPrices(): List<Spot> {
            val spotPrices = File("C:\\Dev\\option\\src\\main\\kotlin\\net\\corda\\option\\oracle\\service\\example.spots.txt").readLines()
            return spotPrices.map(this::parseSpot).toList()
        }
        ...
        /** Parses a string of the form "IBM 13:30" into a [SpotOf] */
        fun parseSpotOf(key: String): SpotOf {
            val words = key.split(' ')
            val time = words.last()
            val name = words.dropLast(1).joinToString(" ")
            return SpotOf(name, Instant.parse(time))
        }

        // Return spot for a given stock.
        fun query(spotOf: SpotOf): Spot {
            return spots.filter { it.of == spotOf }.single()
        }
        ...
        fun sign(ftx: FilteredTransaction): DigitalSignature.LegallyIdentifiable {
            ...
            fun commandValidator(elem: Command): Boolean {
                ...
                return query(exerciseCommand.spot.of) == exerciseCommand.spot
            }
